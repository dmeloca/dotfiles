...
local bar_font = "JetBrainsMonoNerdFont:style=Bold:size=12"

-- Define your blocks
local blocks = {
    oxwm.bar.block.shell({
        format = "{}",
        command = "$HOME/.config/oxwm/scripts/network.sh",
        interval = 30,
        color = colors.red,
        underline = true,
    }),
...
oxwm.key.chord({
    { { modkey }, "Space" },
    { {},         "Escape" }
}, oxwm.spawn({ "sh", "-c", "$HOME/.config/oxwm/scripts/powermenu.sh" }))

oxwm.key.chord({
    { { modkey }, "Space" },
    { {},         "S" }
}, oxwm.spawn({ "sh", "-c", "$HOME/.config/oxwm/scripts/screen.sh" }))
